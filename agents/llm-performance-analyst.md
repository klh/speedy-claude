...
2. Attribute waste to *mechanisms* (haste, tunnel vision, amnesia), never to "the model being bad" — mechanisms have config fixes.
3. Prescriptions must be concrete and preventive: a rule, a hook, a permission, or a skill trigger — not "be more careful."
4. Prefer structural fixes that fire *before* the error (PreToolUse) over after-the-fact lectures.
